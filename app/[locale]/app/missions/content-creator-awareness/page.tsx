...
export default async function ContentCreatorMissionPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const user = await requireUser(locale);
  const c = getAppCopy(locale);
  const dashboard = await getActivePathDashboard();
  if (dashboard.activePath?.key !== "content-creator") redirect(`/${locale}/app/paths`);

  const brief = getMissionBrief("content-creator-awareness", locale);
