...
export async function GET(_request: Request, { params }: { params: Promise<{ shareId: string }> }) {
  ...
  const { data, error } = await admin.rpc("read_shared_proof", { p_share_id: shareId, p_session_hash: await sha256(session) });
  if (error || !data) return problem(410, "share-unavailable", "This proof link is expired or revoked.");
  return ok(data);
}
